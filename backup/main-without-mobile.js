// Тоже рабочий скрипт, который не дублирует скачивание еще и под мобилку. 
...
async function runQueue() {
  ...
  try {
    ...
    // --- НАСТРОЙКА ГЕОМЕТРИИ СТРАНИЦЫ (А4 или Смартфон) ---
    let pdfOptions = {
      printBackground: true,
    };

    if (format === "Mobile") {
      // Оставляем физические габариты мобильного экрана
      pdfOptions.width = "115mm";
      pdfOptions.height = "185mm";

      // ИСПРАВЛЕНИЕ: Сужаем внешние поля до минимума (4мм вместо 8мм)
      pdfOptions.margin = {
        top: "8mm",
        right: "4mm",
        bottom: "8mm",
      ...
      // ИСПРАВЛЕНИЕ: Обнуляем padding у body и ставим умеренный шрифт 15px для идеальной Readable line
      ...
      // Стандартная десктопная страница
