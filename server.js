/**
 * CLEAN PDF ENGINE - FINAL PRODUCTION VERSION
 * * Описание:
 * Сервис для преобразования web-статей в чистый PDF (Readability + Puppeteer).
 * * Ключевые особенности этой версии:
 * 1. Bypass Protection: Полная имитация браузера с подменой Referer/Origin и
 * обходом блокировок CDN-серверов (актуально даже для таких
 * капризных сервисов как Substack/Medium).
 ...
 * 4. Performance: Оптимизировано под персональное использование, приоритет
 * отдан 100% целостности контента над скоростью генерации.
 *
 * * Сравнение с предыдущими итерациями:
 * - В ранних версиях: Скрипт просто парсил HTML и «печатал» его. Это приводило
 * к появлению «пустых мест» вместо картинок, так как браузер не успевал
 * их скачать, а CDN-серверы блокировали запросы без заголовка Referer.
 * - Что изменилось сейчас:
 * 1. Теперь мы принудительно раскрываем Lazy-loading (заменяем data-src на src).
 * 2. Имитируем «живого» пользователя (Referer/Origin/User-Agent).
 * 3. Ввели принудительную синхронную загрузку (eager) и «бесконечный» Viewport,
 * благодаря чему скрипт «видит» всю статью целиком сразу, что заставляет
 * движок Chromium скачивать абсолютно все медиа-файлы перед созданием PDF.
...
async function runQueue() {
  ...
  try {
    ...
    images.forEach((img) => {
      ...
      // НАСТОЯЩИЙ ФИКС ДЛЯ ГЛУБОКИХ КАРТИНОК:
      // Уничтожаем "ленивую" загрузку Chrome, заставляя его качать всё немедленно (eager)
      img.removeAttribute("loading");
      img.setAttribute("loading", "eager");
      img.removeAttribute("decoding"); // Отключаем асинхронное декодирование пикселей
    });
    ...
    // ФИКС ДЛЯ ОБМАНА ЛЕНИВОЙ ЗАГРУЗКИ:
    // Делаем окно рендеринга экстремально высоким, чтобы все картинки "подумали", что они на первом экране
    ...
    if (format === "Mobile") {
      pdfOptions.width = "115mm";
      pdfOptions.height = "185mm";
      pdfOptions.margin = {
        top: "8mm",
        right: "4mm",
        bottom: "8mm",
      ...
      await renderPage.addStyleTag({
        content: `
          body { font-size: 15px !important; line-height: 1.55 !important; padding: 0 !important; margin: 0 auto !important;}
          h1 { font-size: 1.6rem !important; margin-bottom: 0.4em !important; }
          h2 { font-size: 1.25rem !important; margin-top: 1.3em !important; }
          img { margin: 16px auto !important; }
        `,
      });
    } else {
      pdfOptions.format = format === "Letter" ? "Letter" : "A4";
    }

    const pdfBuffer = await renderPage.pdf(pdfOptions);
